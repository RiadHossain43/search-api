...
class Filters {
  ...
  formatOperators() {
    ...
    this.queryStr = currentString.replace(/\b(or|gt|gte|lt|options|lte|in|nin|ne|equals|all|regex)\b/g, key => `$${key}`)
    return this
  }
  removeOptions() {
    let removeItems = ['page', 'size', 'sort', 'tenant', 'threshold', "createdBy", 'userId', 'roleName']
  ...
  search() {
    ...
    let search = new RegExp(this.qry.clientSearch && this.qry.clientSearch, "i")
    let searchQuery = this.qry.$or || []
    this.qry.$or = [...searchQuery, ...this.searchFields.map(field => {
      return {
        [field]: { $regex: this.qry.clientSearch && this.qry.clientSearch, $options: 'i' }
      }
    })]
    delete this.qry['clientSearch']
    this.queryStr = JSON.stringify(this.qry)
