...
exports.getResults = async (req, res) => {
  ...
  let filter = new Filters(req.query, { searchFields: ['name'] }).build().query()
  let query = { ...filter }
  const [error, queryResults] = await asynchronously(Company.paginate(query, options))
  res.status(200).json({
...
exports.seed = async (req, res) => {
  ...
  const [error, createdData] = await asynchronously(Company.create({
    name,
    founder,
    description,
    website,
