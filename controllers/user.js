...
exports.getUserInfo = async (req, res) => {
  try {
    ...
    res.status(200).json({ success: { message: "user retrived successfully", data: user } })
  } catch (err) {
    console.error(err)
  }
}
    ...
    jwt.verify(token, process.env.JWT_KEY, async (error, decoded) => {
      if (error) {
        ...
        try {
          const salt = await bcrypt.genSalt(10)
          let hashPassword = await bcrypt.hash(password, salt)
          await User.findOneAndUpdate({ _id: decoded.id }, { $set: { password: hashPassword, resetToken: '' } })
