...
exports.loginAndGetToken = async (req, res) => {
  try {
    ...
    const payload = { user }
    jwt.sign(payload, process.env.JWT_KEY, { expiresIn: '5 days' }, (err, token) => {
      if (err) throw err
...
exports.validateRegistration = async (req, res) => {
  try {
    ...
    let user = await User.findOne({ email })
    if (user)
      return res.status(400).json({ error: { message: "This user is already taken." } })
    let payload = {
      name,
      email,
      password
    }
    jwt.sign(payload, process.env.JWT_KEY, { expiresIn: 600 }, async (err, token) => {
      if (err) throw err
      try {
...
exports.forgotPassword = async (req, res) => {
  try {
    ...
    jwt.sign(payload, process.env.JWT_KEY, { expiresIn: 600 }, async (err, token) => {
      ...
      try {
        // send email to user ...
        res.status(200).json({ success: { message: 'email sent', data: token } })
      } catch (err) {
